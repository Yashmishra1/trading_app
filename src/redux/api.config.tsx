...
appAxios.interceptors.request.use(async config => {
  const app_access_token = token_storage.getString('access_token');
  if (app_access_token) {
...
socketAxios.interceptors.request.use(async config => {
  const socket_access_token = token_storage.getString('access_token');
...
export const refresh_tokens = async (
  ...
  try {
    ...
    resetAndNavigate(type === 'app' ? 'LoginScreen' : 'AuthVerificationScreen');
  }
};
